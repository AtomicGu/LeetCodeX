...
class Solution:
    def minSkips(self, dist: List[int], speed: int, hoursBefore: int) -> int:
        ...
        # ! 档次算法没错，但是这题有一个奇怪的对齐机制
        # ! 必须正着构建档次表才能解决对齐机制的问题
        def calc_grade_table(
            end=len(cost),
            ...
            prices, grades = calc_grade_table(end - 1)
            this_cost = cost[end - 1]

            ite = iter(zip(prices, grades))
            price, grade = next(ite)
            if price.denominator == 1:
                new_prices = [price + this_cost]
                new_grades = [grade]
            else:
                new_prices = [price + this_cost, ceil(price) + this_cost]
            ...
            for price, grade in ite:
                ...
                if price.denominator == 1:
                    new_prices.append(price + this_cost)
                    new_grades.append(grade)
                else:
                    a = price + this_cost
                    if a < new_prices[-1]:
                        new_prices[-1] = a  # grade+1
                    new_prices.append(ceil(price) + this_cost)
                    new_grades.append(grade)
